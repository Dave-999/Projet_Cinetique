...
La vitesse d'apparition de PMMA est donnée par $\frac{d[P]}{dt}=k_{t}[R]^2+k_{s}[R][TrH]$. On obtient alors la fonction à intégrer pour trouver [P]. Si nous négligeons l'impact de l'agent de transfert, on voit de manière immédiate que $[P]=r_i t$. %En développant et mettant les $[R]_{t=0}$ en évidence, on a :
